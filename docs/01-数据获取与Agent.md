...
Agent 只接受 HTTP(S)，拒绝 URL userinfo、私网/环回/链路本地/保留地址和混合公网私网解析。HTTP 引擎把校验放进
niquests 实际使用的解析器，连接器拿到的就是已校验记录，避免“先检查、连接时再次解析”的 DNS rebinding 窗口。
Browser 引擎把显式 `allowed_domains` 和首跳 Host 解析为公网地址并写入 Chromium host-resolver rules；未显式列出的
子域请求会被拦截，需要的子域必须单独加入白名单。首跳、重定向、最终 URL 和浏览器子请求都会再次校验。

应用层校验不是网络隔离的替代品。生产 Agent 仍应在独立网段运行，并由防火墙或出口代理拒绝 RFC 1918、链路本地、
云元数据地址和控制面管理网段；这样即使浏览器或依赖库未来出现绕过，内核/网络层仍会拒绝连接。
